    ...
    for target_rate=[62.5, 125, 250]
        ...
        for ds_idx=min_ds_idx:-1:max_ds_idx    
            for phase=0:10:ds_idx-1
                ...
                save(fullfile(pwd, 'dataset', 'augmented_2mul', strcat('fall_tr_aug_', num2str(target_rate)), file_name), 'result');
%                 whos('-file',fullfile(pwd, 'dataset', 'augmented_2mul', strcat('fall_tr_aug_', num2str(target_rate)), file_name))
            end
        end
    end
end
    ...
    for target_rate=[62.5, 125, 250]
        ...
        for ds_idx=min_ds_idx:-1:max_ds_idx    
            for phase=0:10:ds_idx-1
                ...
                file_name = strcat(new_name, '_ph',num2str(phase, '%02d'), '_', num2str(ds_idx, '%02d'),'to', num2str(sampling_rate/target_rate, '%02d'), '.mat')
                save(fullfile(pwd, 'dataset', 'augmented_2mul', strcat('nonfall_tr_aug_', num2str(target_rate)), file_name), 'result');
%                 whos('-file',fullfile(pwd, 'dataset', 'augmented_2mul', strcat('nonfall_tr_aug_', num2str(target_rate)), file_name))
            end
        end
    end
end